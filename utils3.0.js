...
var utils = (function () {
    ...
    function tab(box,onxxx,selectedIndex){
        onxxx=onxxx||'onmouseover';
        ...
        for(var i=0;i<lis.length;i++){
            lis[i][onxxx]= function () {
                var siblings=siblings(this);
                for(var j=0;j<siblings.length;j++){
                    removeClass(siblings[j],'selected');
                }
                addClass(this,'selected');
                var index=index(this);
                for(var k=0;k<divs.length;k++){
                    k===index?addClass(divs[k],'selected'):removeClass(divs[k],'selected');
                }
            }
        }
